...
class TreeList extends React.Component {
  ...
  expandTree(listItems, activeItemIndex){
    if (typeof activeItemIndex !== 'undefined'
        && activeItemIndex !== null 
        && activeItemIndex >=0){

           let item = listItems[activeItemIndex]; 
           let parentItems = [];

           if (item.children){
                parentItems.push(activeItemIndex);
           }
           ...
           while(item){
                ...
                if (typeof parentIndex !== 'undefined'){
                    parentItems.push(parentIndex);

                    item=listItems[parentIndex];
                }else{
                    break;
                }
           }

           return parentItems.reverse();
    }
  ...
  componentDidUpdate(prevProps){
    if(prevProps.activeListItem !== this.props.activeListItem){
        this.setState({
  ...
  render() {
    ...
    const expandedListItems = this.expandTree(listItems, activeListItem);
